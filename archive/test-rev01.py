...
v1 = model.parts['transFlange'].edges.findAt(((0,0.0,0.25),))
v2 = model.parts['transFlange'].edges.findAt(((1.5,0.0,0.25),))
v3 = model.parts['transFlange'].edges.findAt(((3.0,0.0,0.25),))
stiffEdges = v1 + v2 + v3
model.parts['transFlange'].Set(edges=stiffEdges, name='Set-transFlange-edge')
...
model.Tie(
    ...
    name='Constraint-3',
    positionToleranceMethod=COMPUTED,
    secondary=model.rootAssembly.instances['transFlange-1'].sets['Set-transFlange-edge'],
    thickness=ON,
    tieRotations=ON)