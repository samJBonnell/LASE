...
OPMODE = True
if OPMODE is True:
    with open("temp\\input.csv", 'r') as f:
    ...
    PlateMesh = float(panelVariables[12])
    TransverseStiffenerMesh = float(panelVariables[13])
    TransverseFlangeMesh = float(panelVariables[14])
    LongitudinalStiffenerMesh = float(panelVariables[15])
    LongitudinalFlangeMesh = float(panelVariables[16])

    PanelWidth = float(panelVariables[17])
    PanelLength = float(panelVariables[18])
...
model.parts['plate'].seedPart(deviationFactor=0.1,minSizeFactor=0.1, size=PlateMesh)
model.parts['plate'].generateMesh()

model.parts['transStiff'].seedPart(deviationFactor=0.1,minSizeFactor=0.1, size=TransverseStiffenerMesh)
model.parts['transStiff'].generateMesh()

model.parts['transFlange'].seedPart(deviationFactor=0.1,minSizeFactor=0.1, size=TransverseFlangeMesh)
model.parts['transFlange'].generateMesh()

model.parts['longStiff'].seedPart(deviationFactor=0.1, minSizeFactor=0.1, size=LongitudinalStiffenerMesh)
model.parts['longStiff'].generateMesh()

model.parts['longFlange'].seedPart(deviationFactor=0.1, minSizeFactor=0.1, size=LongitudinalFlangeMesh)
